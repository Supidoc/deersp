...
volatile uint32_t rxXferCounter = 0;
volatile uint32_t txXferCounter = 0;
...
HAL_StatusTypeDef codecSetup()
{
	...
	{ .BclkOutput = false, .WclkOutput = false, .DoutTristate = true,
...
//	  		status = USER_AIC3X_WritePllProgramming(&aic3xHandle, &pllProgramming);
	...
	AIC3X_ADCMixControl_TypeDef adcMixControl =
			{ .LeftADC_MIC2LGain = 0, .LeftADC_MIC2RGain = 0b1111,
					.RightADC_MIC2LGain = 0b1111, .RightADC_MIC2RGain = 0,
					.LeftADC_MIC1LPDifferential = false, .LeftADC_MIC1LPGain =
							0b1111, .LeftADC_PowerUp = true,
					.LeftADC_PGASoftStepping = 0b00,
					.LeftADC_MIC1RPDifferential = false, .LeftADC_MIC1RPGain =
							0b1111, .RightADC_MIC1RPDifferential = false,
...
void MPU_Config(void)
{
  ...
  */
  ...
  MPU_InitStruct.BaseAddress = 0x38002000;
  ...
  */
  ...
  MPU_InitStruct.BaseAddress = 0x38004000;
